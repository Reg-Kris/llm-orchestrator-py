"""
Session manager interface and hybrid session management
Provides unified interface for different session backends
"""
...
class HybridSessionManager(SessionManagerInterface):
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning("PostgreSQL session support not available")
        
        self.use_postgres = use_postgres and self.postgres_available
        
        # Initialize managers
        self.postgres_manager: Optional[Any] = None
        self.redis_manager: Optional[Any] = None
        self.memory_manager: Optional[Any] = None
        
        if self.use_postgres and self.postgres_available:
            from pyairtable_common.database.session_repository import PostgreSQLSessionRepository
        ...
        if self.use_redis_fallback:
            ...
            self.redis_manager = RedisSessionManager(redis_url, redis_password, session_timeout)
        
        # Always have in-memory as final fallback
        from .memory_backend import InMemorySessionManager
        self.memory_manager = InMemorySessionManager(session_timeout)
